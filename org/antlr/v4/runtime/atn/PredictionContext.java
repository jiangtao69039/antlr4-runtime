...
public abstract class PredictionContext {
	...
	private static final ThreadLocal<TimeoutChecker> TIMEOUT_CHECKER = new ThreadLocal<TimeoutChecker>();

	public static void setTimeoutChecker(TimeoutChecker checker) {
		if (checker == null) {
			TIMEOUT_CHECKER.remove();
		}
		else {
			TIMEOUT_CHECKER.set(checker);
		}
	}

	public static void checkTimeout() {
		TimeoutChecker checker = TIMEOUT_CHECKER.get();
		if (Thread.currentThread().isInterrupted()) {
			throw new java.util.concurrent.CancellationException("Parsing interrupted");
		}
		if (checker != null) {
			checker.check();
		}
